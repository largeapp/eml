...
for n_fold, (train_val, test) in enumerate(skf1.split(labels, labels)):
    ...
    import shap
    # shap.initjs()
    explainer = shap.KernelExplainer(interpreting, X_test)
    shap_values = explainer.shap_values(X_test)
    np.save(outputdir + '/shap_values_%.02i.npy'%(n_fold), shap_values)
    np.save(outputdir + '/test_%02i.npy'%(n_fold), X_test)
    yhat = interpreting(X_test)
